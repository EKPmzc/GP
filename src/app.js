...
const TAB_IDS = ['mosaic', 'glitch', 'aurora', 'particle', 'audio', 'nebula', 'wave', 'api', 'flux', 'sketch', 'prism', 'ai'];
...
const fluxControls = {
  density: document.getElementById('fluxDensity'),
  densityNumber: document.getElementById('fluxDensityNumber'),
  flow: document.getElementById('fluxFlow'),
  flowNumber: document.getElementById('fluxFlowNumber'),
  glow: document.getElementById('fluxGlow'),
  glowNumber: document.getElementById('fluxGlowNumber'),
  snap: document.getElementById('fluxSnap')
};

const sketchControls = {
  edge: document.getElementById('sketchEdge'),
  edgeNumber: document.getElementById('sketchEdgeNumber'),
  paper: document.getElementById('sketchPaper'),
  halftone: document.getElementById('sketchHalftone'),
  invert: document.getElementById('sketchInvert'),
  snap: document.getElementById('sketchSnap')
};

const prismControls = {
  layers: document.getElementById('prismLayers'),
  layersNumber: document.getElementById('prismLayersNumber'),
  saturation: document.getElementById('prismSaturation'),
  saturationNumber: document.getElementById('prismSaturationNumber'),
  drift: document.getElementById('prismDrift'),
  driftNumber: document.getElementById('prismDriftNumber'),
  snap: document.getElementById('prismSnap')
};

const aiControls = {
  generate: document.getElementById('aiPalette'),
  apply: document.getElementById('aiApplyPalette'),
  model: document.getElementById('aiModel'),
  blend: document.getElementById('aiBlend'),
  blendNumber: document.getElementById('aiBlendNumber'),
  status: document.getElementById('aiStatus'),
  preview: document.getElementById('aiPreview')
};
...
const audioControls = {
  ...
  particleOrbit: document.getElementById('audioParticleOrbit'),
  particleOrbitNumber: document.getElementById('audioParticleOrbitNumber'),
  particleTrail: document.getElementById('audioParticleTrail'),
  particleTrailNumber: document.getElementById('audioParticleTrailNumber'),
  ...
  fluxFlow: document.getElementById('audioFluxFlow'),
  fluxFlowNumber: document.getElementById('audioFluxFlowNumber'),
  fluxGlow: document.getElementById('audioFluxGlow'),
  fluxGlowNumber: document.getElementById('audioFluxGlowNumber'),
  sketchEdge: document.getElementById('audioSketchEdge'),
  sketchEdgeNumber: document.getElementById('audioSketchEdgeNumber'),
  sketchInk: document.getElementById('audioSketchInk'),
  sketchInkNumber: document.getElementById('audioSketchInkNumber'),
  prismLayers: document.getElementById('audioPrismLayers'),
  prismLayersNumber: document.getElementById('audioPrismLayersNumber'),
  prismBloom: document.getElementById('audioPrismBloom'),
  prismBloomNumber: document.getElementById('audioPrismBloomNumber'),
  aiBlend: document.getElementById('audioAiBlend'),
  aiBlendNumber: document.getElementById('audioAiBlendNumber')
};
...
const audioState = {
  ...
  config: {
    ...
    particle: { orbit: 1, trail: 0.4 },
    nebula: { density: 0.5, swirl: 0.8 },
    wave: { amplitude: 0.7, hue: 120 },
    flux: { flow: 1, glow: 0.5 },
    sketch: { edge: 0.6, ink: 0.4 },
    prism: { layers: 0.5, bloom: 0.7 },
    ai: { blend: 0.5 }
  }
};
...
function switchTab(tabId) {
  ...
  if (tabId === 'mosaic' && previewActive) {
    ...
    renderFlux(performance.now());
  } else if (tabId === 'sketch') {
    renderSketch();
  } else if (tabId === 'prism') {
    renderPrism(performance.now());
  } else if (tabId === 'ai') {
    refreshAiPreview();
  }
}
...
function initFluxControls() {
  if (!fluxControls.density) return;
  linkSliderNumber(fluxControls.density, fluxControls.densityNumber, (value) => {
    fluxState.density = clamp(parseInt(value, 10), 1000, 40000);
    fluxState.particles = [];
    if (activeTab === 'flux') {
      renderFlux(performance.now());
    }
  });
  linkSliderNumber(fluxControls.flow, fluxControls.flowNumber, (value) => {
    fluxState.flow = parseFloat(value);
    if (activeTab === 'flux') {
      renderFlux(performance.now());
    }
  });
  linkSliderNumber(fluxControls.glow, fluxControls.glowNumber, (value) => {
    fluxState.glow = clamp(parseFloat(value), 0, 1.5);
    if (activeTab === 'flux') {
      renderFlux(performance.now());
    }
  });
  fluxControls.snap?.addEventListener('click', () => {
    renderFlux(performance.now());
    captureSnapshotToMosaic();
    setStatus('Flux field captured to mosaic');
  });
}

function initSketchControls() {
  if (!sketchControls.edge) return;
  linkSliderNumber(sketchControls.edge, sketchControls.edgeNumber, (value) => {
    sketchState.edge = parseFloat(value);
    sketchState.needsRedraw = true;
    if (activeTab === 'sketch') {
      renderSketch();
    }
  });
  sketchControls.paper.addEventListener('change', () => {
    sketchState.paper = sketchControls.paper.value;
    sketchState.needsRedraw = true;
    if (activeTab === 'sketch') {
      renderSketch();
    }
  });
  sketchControls.halftone.addEventListener('change', () => {
    sketchState.halftone = sketchControls.halftone.checked;
    sketchState.needsRedraw = true;
    if (activeTab === 'sketch') {
      renderSketch();
    }
  });
  sketchControls.invert.addEventListener('change', () => {
    sketchState.invert = sketchControls.invert.checked;
    sketchState.needsRedraw = true;
    if (activeTab === 'sketch') {
      renderSketch();
    }
  });
  sketchControls.snap?.addEventListener('click', () => {
    renderSketch();
    captureSnapshotToMosaic();
    setStatus('Sketch transferred to mosaic');
  });
}

function initPrismControls() {
  if (!prismControls.layers) return;
  linkSliderNumber(prismControls.layers, prismControls.layersNumber, (value) => {
    prismState.layers = clamp(parseInt(value, 10), 2, 20);
    if (activeTab === 'prism') {
      renderPrism(performance.now());
    }
  });
  linkSliderNumber(prismControls.saturation, prismControls.saturationNumber, (value) => {
    prismState.saturation = clamp(parseFloat(value), 0, 3);
    if (activeTab === 'prism') {
      renderPrism(performance.now());
    }
  });
  linkSliderNumber(prismControls.drift, prismControls.driftNumber, (value) => {
    prismState.drift = clamp(parseFloat(value), 0, 6);
    if (activeTab === 'prism') {
      renderPrism(performance.now());
    }
  });
  prismControls.snap?.addEventListener('click', () => {
    renderPrism(performance.now());
    captureSnapshotToMosaic();
    setStatus('Prism bloom captured to mosaic');
  });
}

function initAiControls() {
  if (!aiControls.generate) return;
  linkSliderNumber(aiControls.blend, aiControls.blendNumber, (value) => {
    aiState.blend = clamp(parseFloat(value), 0, 1);
  });
  aiControls.model.addEventListener('change', () => {
    aiState.model = aiControls.model.value;
  });
  aiControls.generate.addEventListener('click', requestAiPalette);
  aiControls.apply.addEventListener('click', applyAiPalette);
  refreshAiPreview();
}
...
function initAudioControls() {
  ...
  audioControls.target.addEventListener('change', () => {
    ...
    if (activeTab === 'audio') {
      if (audioState.target === 'mosaic') {
        ...
        renderGlitch();
      } else if (audioState.target === 'particle') {
        renderParticleScene(performance.now());
      } else if (audioState.target === 'nebula') {
        renderNebula(performance.now());
      } else if (audioState.target === 'wave') {
        renderWave(performance.now());
      } else if (audioState.target === 'flux') {
        renderFlux(performance.now());
      } else if (audioState.target === 'sketch') {
        sketchState.needsRedraw = true;
        renderSketch();
      } else if (audioState.target === 'prism') {
        renderPrism(performance.now());
      } else if (audioState.target === 'ai') {
        refreshAiPreview();
      }
    }
  });
  ...
  linkSliderNumber(audioControls.particleOrbit, audioControls.particleOrbitNumber, (value) => {
    audioState.config.particle.orbit = parseFloat(value);
    if (audioState.target === 'particle') renderParticleScene(performance.now());
  });
  linkSliderNumber(audioControls.particleTrail, audioControls.particleTrailNumber, (value) => {
    audioState.config.particle.trail = parseFloat(value);
  });
  ...
  linkSliderNumber(audioControls.fluxFlow, audioControls.fluxFlowNumber, (value) => {
    audioState.config.flux.flow = parseFloat(value);
  });
  linkSliderNumber(audioControls.fluxGlow, audioControls.fluxGlowNumber, (value) => {
    audioState.config.flux.glow = parseFloat(value);
  });
  linkSliderNumber(audioControls.sketchEdge, audioControls.sketchEdgeNumber, (value) => {
    audioState.config.sketch.edge = parseFloat(value);
    if (audioState.target === 'sketch') {
      sketchState.needsRedraw = true;
      renderSketch();
    }
  });
  linkSliderNumber(audioControls.sketchInk, audioControls.sketchInkNumber, (value) => {
    audioState.config.sketch.ink = parseFloat(value);
  });
  linkSliderNumber(audioControls.prismLayers, audioControls.prismLayersNumber, (value) => {
    audioState.config.prism.layers = parseFloat(value);
  });
  linkSliderNumber(audioControls.prismBloom, audioControls.prismBloomNumber, (value) => {
    audioState.config.prism.bloom = parseFloat(value);
  });
  linkSliderNumber(audioControls.aiBlend, audioControls.aiBlendNumber, (value) => {
    audioState.config.ai.blend = clamp(parseFloat(value), 0, 1);
  });
...
function initInfo() {
  const infoCopy = {
    ...
    flux: 'Flux Field spins GPU particles into magnetic ribbons—set density, flow, and glow for cinematic motion captures.',
    sketch: 'Sketchbook turns your source into inked outlines with adjustable edge strength, paper tint, halftones, and invertible inks.',
    prism: 'Prism Bloom layers palette-driven gradients over the source, adding chromatic drift and soft bloom for dreamy composites.',
    ai: 'AI Studio requests Colormind palettes (with offline fallbacks) and blends them into your project with tunable influence.',
    audio: 'Audio Reactor syncs beats to every filter—pick a target and adjust how each mode responds to level, bass, and treble energy.'
  };
...
function tick(ts) {
  ...
  if (activeTab === 'mosaic') {
    ...
    renderPrism(ts);
  } else if (activeTab === 'sketch' && sketchState.needsRedraw) {
    renderSketch();
    sketchState.needsRedraw = false;
  }
  updateFPS(ts);
  requestAnimationFrame(tick);
}
...
function renderAudioDriven(ts) {
  ...
  if (target === 'ai') {
    if (aiControls && aiControls.blend) {
      const dynamicBlend = clamp(
        (aiState.blend ?? parseFloat(aiControls.blend.value || '0.5')) +
          (audioState.level - 0.5) * (audioState.config.ai.blend || 0),
        0,
        1
      );
      aiControls.blend.value = dynamicBlend.toFixed(2);
      aiControls.blendNumber.value = dynamicBlend.toFixed(2);
      aiState.blend = dynamicBlend;
    }
    refreshAiPreview();
  }
}
...
function renderParticleScene(timestamp = performance.now()) {
  const isAudioPreview = activeTab === 'audio' && audioState.target === 'particle';
  if (!isAudioPreview && activeTab !== 'particle') return;
  if (!particleRenderer) {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
  ...
  let speedScale = particleState.speed;
  let trailAlpha = 0.15;
  if (audioState.running && audioState.target === 'particle') {
    speedScale = particleState.speed + audioState.level * audioState.config.particle.orbit;
    if (particleState.trails) {
      trailAlpha = clamp(0.05 + audioState.config.particle.trail * audioState.level, 0.05, 0.5);
    }
  }

  if (particleState.trails) {
    ctx.fillStyle = `rgba(5,7,12,${trailAlpha})`;
    ctx.fillRect(0, 0, canvas.width, canvas.height);
  } else {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
  ...
  for (let i = 0; i < particleState.particles.length; i++) {
    ...
    particle.angle += dt * speedScale * particle.speed;
...
function renderNebula(timestamp = performance.now()) {
  const isAudioPreview = activeTab === 'audio' && audioState.target === 'nebula';
  if (!isAudioPreview && activeTab !== 'nebula') return;
  if (!particleRenderer) {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.fillStyle = '#0d1320';
    ctx.fillRect(0, 0, canvas.width, canvas.height);
  ...
  let speed = nebulaState.speed;
  let swirl = nebulaState.swirl;
  let activeCount = nebulaState.cloud.length;
  if (audioState.running && audioState.target === 'nebula') {
    speed = clamp(nebulaState.speed * (1 + audioState.level * audioState.config.nebula.density), 0, 5);
    swirl = clamp(nebulaState.swirl + audioState.bass * audioState.config.nebula.swirl, 0, 6);
    activeCount = Math.min(
      nebulaState.cloud.length,
      Math.floor(nebulaState.cloud.length * (1 + audioState.level * audioState.config.nebula.density))
    );
  }
...
function renderWave(timestamp = performance.now()) {
  const isAudioPreview = activeTab === 'audio' && audioState.target === 'wave';
  if (!isAudioPreview && activeTab !== 'wave') return;
  if (!particleRenderer) {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
  ...
  const height = canvas.height;
  ctx.clearRect(0, 0, width, height);
  let amplitude = waveState.amplitude;
  let hueBase = waveState.hue;
  if (audioState.running && audioState.target === 'wave') {
    amplitude = clamp(
      waveState.amplitude * (1 + audioState.level * audioState.config.wave.amplitude),
      0,
      160
    );
    hueBase = (hueBase + audioState.treble * audioState.config.wave.hue) % 360;
  }
  const gradient = ctx.createLinearGradient(0, 0, width, height);
  gradient.addColorStop(0, `hsla(${(hueBase + 20) % 360}, 78%, 55%, 1)`);
  gradient.addColorStop(1, `hsla(${(hueBase + 200) % 360}, 65%, 14%, 1)`);
  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, width, height);
  ...
  const hueShift = (hueBase % 360) / 360;
  const shapeId = shapeLibrary[state.shapeName]?.index ?? shapeLibrary.Circle?.index ?? 0;
...
function seedFluxParticles(force = false) {
  const desired = clamp(Math.floor(fluxState.density), 1000, 35000);
  if (!force && fluxState.particles.length === desired && fluxState.lastSeed === state.seed) {
    return;
  }
  const rng = mulberry32((state.seed ^ 0x6c796765) + frameSeedOffset);
  const particles = [];
  for (let i = 0; i < desired; i++) {
    particles.push({
      angle: rng() * Math.PI * 2,
      radius: Math.pow(rng(), 1.2),
      speed: 0.45 + rng() * 1.8,
      offset: rng() * Math.PI * 2,
      colorIndex: Math.floor(rng() * 2048)
    });
  }
  fluxState.particles = particles;
  fluxState.lastSeed = state.seed;
}

function renderFlux(timestamp = performance.now()) {
  const isAudioPreview = activeTab === 'audio' && audioState.target === 'flux';
  if (!isAudioPreview && activeTab !== 'flux') return;
  if (!particleRenderer) {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText('WebGL required for Flux Field', canvas.width / 2, canvas.height / 2);
    ctx.restore();
    return;
  }
  seedFluxParticles();
  const dt = fluxState.lastTime ? Math.min(0.05, Math.max(0, (timestamp - fluxState.lastTime) / 1000)) : 0.016;
  fluxState.lastTime = timestamp;
  let flow = fluxState.flow;
  let glow = fluxState.glow;
  if (audioState.running && audioState.target === 'flux') {
    flow = Math.max(0.1, fluxState.flow + audioState.level * audioState.config.flux.flow);
    glow = clamp(fluxState.glow + audioState.bass * audioState.config.flux.glow, 0, 2);
  }
  const trailAlpha = clamp(0.08 + glow * 0.12, 0.05, 0.45);
  ctx.fillStyle = `rgba(6, 10, 18, ${trailAlpha})`;
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  const paletteHex = state.palette.length ? state.palette : palettePresets.Aurora;
  const palette = paletteHex.map((hex) => ({ ...hexToRgb(hex), a: 255 }));
  const centerX = canvas.width / 2;
  const centerY = canvas.height / 2;
  const baseRadius = Math.min(centerX, centerY) * 0.92;
  const shapeId = shapeLibrary[state.shapeName]?.index ?? shapeLibrary.Circle?.index ?? 0;
  const shimmer = Math.sin(timestamp * 0.0013) * 0.5 + 0.5;
  const shapes = [];
  const paletteCount = palette.length || 1;
  for (let i = 0; i < fluxState.particles.length; i++) {
    const particle = fluxState.particles[i];
    particle.angle += dt * flow * (0.6 + particle.speed);
    particle.offset += dt * flow * 0.4;
    const dist = baseRadius * (0.12 + particle.radius * 0.88);
    const wave = Math.sin(particle.offset + timestamp * 0.0009 * flow);
    const x = centerX + Math.cos(particle.angle) * dist;
    const y = centerY + Math.sin(particle.angle * 0.82 + wave) * dist;
    const color = palette[particle.colorIndex % paletteCount] || palette[0];
    const sparkle = 0.6 + 0.4 * Math.sin(timestamp * 0.002 + particle.offset * 1.3 + i * 0.01);
    const alpha = clamp((0.5 + glow * 0.4 + shimmer * 0.2) * 255, 60, 255);
    shapes.push({
      x,
      y,
      radius: 3.2 + dist * 0.002 + sparkle * 2.4,
...
function renderSketch() {
  const isAudioPreview = activeTab === 'audio' && audioState.target === 'sketch';
  if (!isAudioPreview && activeTab !== 'sketch') return;
  const ready = ensureSourceReady();
  if (!ready) {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
  ...
  if (sketchCanvas.width !== sample.width || sketchCanvas.height !== sample.height) {
    sketchCanvas.width = sample.width;
    sketchCanvas.height = sample.height;
  }
  const edgeStrength = Math.max(0, sketchState.edge);
  let dynamicEdge = edgeStrength;
  let inkBoost = 0;
  if (audioState.running && audioState.target === 'sketch') {
    dynamicEdge = edgeStrength * (1 + audioState.level * audioState.config.sketch.edge);
    inkBoost = audioState.bass * audioState.config.sketch.ink;
  }
  ...
  for (let y = 1; y < height - 1; y++) {
    for (let x = 1; x < width - 1; x++) {
      const idx = y * width + x;
      const idx4 = idx * 4;
      const luminanceCenter = toGrayscale({ r: data[idx4], g: data[idx4 + 1], b: data[idx4 + 2] });
      const luminanceRight = toGrayscale({ r: data[idx4 + 4], g: data[idx4 + 5], b: data[idx4 + 6] });
      const luminanceLeft = toGrayscale({ r: data[idx4 - 4], g: data[idx4 - 3], b: data[idx4 - 2] });
      const luminanceUp = toGrayscale({ r: data[idx4 - width * 4], g: data[idx4 - width * 4 + 1], b: data[idx4 - width * 4 + 2] });
      const luminanceDown = toGrayscale({ r: data[idx4 + width * 4], g: data[idx4 + width * 4 + 1], b: data[idx4 + width * 4 + 2] });
      const gx = luminanceRight - luminanceLeft;
      const gy = luminanceDown - luminanceUp;
      const magnitude = Math.sqrt(gx * gx + gy * gy);
      edges[idx] = magnitude;
    }
  }
  const edgeMax = edges.reduce((max, value) => Math.max(max, value), 1);
  const paper = hexToRgb(sketchState.paper || '#f2ede3');
  const halftone = sketchState.halftone;
  const invert = sketchState.invert;
  const imageData = sketchCtx.createImageData(width, height);
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      const idx = y * width + x;
      const value = edges[idx] / edgeMax;
      let ink = clamp(value * dynamicEdge + inkBoost, 0, 1);
      if (halftone) {
        const pattern = (x % 4 === 0 || y % 4 === 0) ? 0.85 : 1;
        ink *= pattern;
      }
      if (invert) {
        ink = 1 - ink;
      }
      const shade = clamp(1 - ink, 0, 1);
      const base = sketchState.paper ? paper : { r: 242, g: 237, b: 227 };
      const r = clamp(base.r * shade, 0, 255);
      const g = clamp(base.g * shade, 0, 255);
      const b = clamp(base.b * shade, 0, 255);
      const id = idx * 4;
      imageData.data[id] = r;
  ...
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  ctx.fillStyle = rgbToCss({ r: paper.r, g: paper.g, b: paper.b, a: 255 });
  ctx.fillRect(0, 0, canvas.width, canvas.height);
  ctx.drawImage(sketchCanvas, 0, 0, sketchCanvas.width, sketchCanvas.height, 0, 0, canvas.width, canvas.height);
  sketchState.needsRedraw = false;
}

function renderPrism(timestamp = performance.now()) {
  const isAudioPreview = activeTab === 'audio' && audioState.target === 'prism';
  if (!isAudioPreview && activeTab !== 'prism') return;
  const ready = ensureSourceReady();
  if (!ready) {
    ctx.clearRect(0, 0, canvas.width, canvas.height);
  ...
  const paletteHex = state.palette.length ? state.palette : palettePresets.Aurora;
  const layersBase = prismState.layers;
  const saturationBase = prismState.saturation;
  let drift = prismState.drift;
  let layers = layersBase;
  let saturation = saturationBase;
  if (audioState.running && audioState.target === 'prism') {
    layers = clamp(layersBase + audioState.level * audioState.config.prism.layers * 10, 2, 20);
    saturation = clamp(saturationBase * (1 + audioState.treble * 0.5), 0, 3);
    drift += audioState.bass * audioState.config.prism.bloom;
  }
  prismState.phase += drift * 0.003;
  ctx.clearRect(0, 0, canvas.width, canvas.height);
  ctx.drawImage(sampleCanvas, 0, 0, sampleCanvas.width, sampleCanvas.height, 0, 0, canvas.width, canvas.height);
  ctx.save();
  ctx.globalCompositeOperation = 'overlay';
  const palette = paletteHex.map((hex) => adjustSaturation(hexToRgb(hex), saturation));
  for (let i = 0; i < Math.round(layers); i++) {
    const color = palette[i % palette.length] || palette[0];
    const gradient = ctx.createLinearGradient(0, 0, canvas.width, 0);
    const offset = (prismState.phase + i / layers) % 1;
    gradient.addColorStop(0, `rgba(${color.r},${color.g},${color.b},0.05)`);
    gradient.addColorStop(0.5, `rgba(${color.r},${color.g},${color.b},0.22)`);
    gradient.addColorStop(1, `rgba(${color.r},${color.g},${color.b},0.05)`);
    ctx.fillStyle = gradient;
    ctx.save();
    ctx.translate(canvas.width / layers * 0.1, 0);
    ctx.rotate(0.002 * (i % 2 === 0 ? 1 : -1));
    ctx.fillRect(-canvas.width * offset, 0, canvas.width * 2, canvas.height);
    ctx.restore();
  }
  ctx.restore();
  ctx.save();
  ctx.globalCompositeOperation = 'screen';
  const rng = mulberry32(state.seed ^ 0x4258f1d2);
  for (let i = 0; i < 8; i++) {
    const radius = (rng() * 0.4 + 0.1) * Math.max(canvas.width, canvas.height);
...
function adjustSaturation(color, multiplier) {
  const hsv = rgbToHsv(color);
  hsv.s = clamp(hsv.s * multiplier, 0, 1);
  const tinted = hsvToRgb(hsv);
  return { r: tinted.r, g: tinted.g, b: tinted.b, a: color.a ?? 255 };
}
...
function rgbArrayToHex(rgb) {
  const [r, g, b] = rgb;
  return rgbToHex({ r: clamp(Math.round(r), 0, 255), g: clamp(Math.round(g), 0, 255), b: clamp(Math.round(b), 0, 255) });
}
...
const fallbackPalettes = {
  picsum: ['#142d4c', '#385170', '#9fd3c7', '#f6f5f5', '#e3f2fd'],
  art: ['#332532', '#644d52', '#f77a52', '#ff974f', '#ffbca2'],
  dog: ['#2d1f1f', '#533a3f', '#a27035', '#f7c873', '#fbead2'],
  ai: ['#1a1423', '#3d2c8d', '#9163cb', '#c996cc', '#f2f2f2'],
  flux: ['#04172b', '#053f5c', '#429ea6', '#a5be00', '#f2f230']
};

function generateFallbackPalette() {
  const rng = mulberry32(state.seed ^ 0x4a39f2d3);
  const centerHue = rng() * 360;
  const spread = 40 + rng() * 160;
  const colors = [];
  for (let i = 0; i < 7; i++) {
    const hue = (centerHue + (i - 3) * (spread / 6)) % 360;
    const saturation = 50 + rng() * 40;
    const lightness = 40 + rng() * 20;
    colors.push(hslToHex(hue, saturation, lightness));
  }
  return colors;
}

function generateProceduralScene(theme, label = 'Procedural Source') {
  const palette = fallbackPalettes[theme] || fallbackPalettes.flux;
  const seedValue = hashSeed(`${theme}-${state.seed}`);
  ...
  for (let i = 0; i < layers; i++) {
    ...
    sampleCtx.fillStyle = color + '33';
    const cx = rng() * width;
    const cy = rng() * height;
    const rx = width * (0.1 + rng() * 0.4);
    const ry = height * (0.1 + rng() * 0.4);
    sampleCtx.save();
    sampleCtx.translate(cx, cy);
    sampleCtx.rotate(rng() * Math.PI * 2);
    sampleCtx.beginPath();
    sampleCtx.ellipse(0, 0, rx, ry, 0, 0, Math.PI * 2);
    sampleCtx.fill();
    sampleCtx.restore();
  }
  lastSource = 'image';
...
async function fetchPicsumImage() {
  ...
  try {
    ...
    apiState.picsum = generateProceduralScene('picsum', 'Picsum Scenic Fallback');
    apiControls.status.textContent += ' – using procedural scenic';
    refreshApiPreview();
    setStatus('Procedural scenic generated');
  }
}
...
async function fetchArtImage() {
  ...
  try {
    ...
    apiState.art = generateProceduralScene('art', 'Gallery Fallback');
    apiControls.status.textContent += ' – gallery fallback ready';
    refreshApiPreview();
    setStatus('Gallery fallback generated');
  }
}
...
async function fetchDogImage() {
  ...
  try {
    ...
    apiState.dog = generateProceduralScene('dog', 'Canine Muse Fallback');
    apiControls.status.textContent += ' – using generative pup';
    refreshApiPreview();
    setStatus('Generated canine fallback');
  }
}
...
async function fetchRemotePalette() {
  ...
  try {
    ...
    const palette = generateFallbackPalette();
    apiState.palette = palette;
    apiControls.status.textContent += ` – fallback palette ready (${palette.length})`;
    refreshApiPreview();
  }
}
...
async function requestAiPalette() {
  if (!aiControls.status) return;
  aiControls.status.textContent = 'Contacting Colormind…';
  const model = aiControls.model?.value || aiState.model || 'default';
  try {
    ...
    const payload = await response.json();
    const colors = Array.isArray(payload.result)
      ? payload.result
          .filter((entry) => Array.isArray(entry) && entry.length === 3)
          .map((entry) => rgbArrayToHex(entry))
      : [];
    if (!colors.length) {
      throw new Error('AI palette empty');
    }
    aiState.palette = colors;
    aiState.model = model;
    aiState.lastStatus = `AI palette ready (${model})`;
    aiControls.status.textContent = aiState.lastStatus;
    refreshAiPreview();
  } catch (error) {
    aiState.palette = generateFallbackPalette();
    aiState.lastStatus = `AI fallback: ${error.message}`;
    aiControls.status.textContent = aiState.lastStatus;
    refreshAiPreview();
  }
}
...
function applyAiPalette() {
  if (!aiState.palette.length) {
    aiControls.status.textContent = 'Generate an AI palette first';
    return;
  }
  const blend = aiState.blend ?? clamp(parseFloat(aiControls.blend?.value || '0.5'), 0, 1);
  const basePalette = state.palette.length ? state.palette.map(hexToRgb) : palettePresets.Warm.map(hexToRgb);
  const aiPalette = aiState.palette.map(hexToRgb);
  const maxLen = Math.max(basePalette.length, aiPalette.length);
  const mixed = [];
  for (let i = 0; i < maxLen; i++) {
    const base = basePalette[i % basePalette.length];
    const target = aiPalette[i % aiPalette.length];
    const r = base.r * (1 - blend) + target.r * blend;
    const g = base.g * (1 - blend) + target.g * blend;
    const b = base.b * (1 - blend) + target.b * blend;
  ...
  mosaicControls.colorMode.value = 'palette';
  queueRender(true);
  aiControls.status.textContent = 'AI palette blended with mosaic';
  aiState.lastStatus = aiControls.status.textContent;
  refreshAiPreview();
}
...
function refreshAiPreview() {
  ...
  if (aiState.palette.length) {
    ...
    aiControls.preview.appendChild(row);
  }
  const status = document.createElement('p');
  status.textContent = aiState.lastStatus || 'Idle';
  aiControls.preview.appendChild(status);
}
...
async function loadImage(src) {
  ...
  try {
    ...
    throw new Error(error.message || 'Image fetch failed');
  }
}
